# Applications and Communications Stages
...
- Entry condition: a worker is preparing to change Applications and Communications.
- Exit condition: `STATE.md` reflects verified code, files, docs, or external-state checks.
- Evidence/checks: read `website/src/components/Contact.tsx`, `HTML landing for claude/tg-notify.php`, `HTML landing for claude/camp.html`; compare with code/tests where relevant; record conflicts as open questions.
