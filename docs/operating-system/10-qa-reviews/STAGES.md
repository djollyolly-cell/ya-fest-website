...
- Entry condition: a worker is preparing to change QA and Reviews.
- Exit condition: `STATE.md` reflects verified code, files, docs, or external-state checks.
- Evidence/checks: read `website/package.json`, `website/e2e/seo.spec.tsx`, `website/e2e/accessibility.spec.tsx`; compare with code/tests where relevant; record conflicts as open questions.
