...
- Entry condition: a worker is preparing to change Migration and Rollout.
- Exit condition: `STATE.md` reflects verified code, files, docs, or external-state checks.
- Evidence/checks: read `website/vite.config.ts`, `HTML landing for claude/sitemap.xml`, `website/public/sitemap.xml`; compare with code/tests where relevant; record conflicts as open questions.
