...
- Entry condition: a worker is preparing to change Backend Runtime.
- Exit condition: `STATE.md` reflects verified code, files, docs, or external-state checks.
- Evidence/checks: read `website/api/contact.ts`, `website/api/__tests__/contact.test.ts`, `HTML landing for claude/tg-notify.php`; compare with code/tests where relevant; record conflicts as open questions.
