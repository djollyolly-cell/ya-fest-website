# Security and Multitenancy Stages
...
- Entry condition: a worker is preparing to change Security and Multitenancy.
- Exit condition: `STATE.md` reflects verified code, files, docs, or external-state checks.
- Evidence/checks: read `website/api/contact.ts`, `HTML landing for claude/tg-notify.php`; compare with code/tests where relevant; record conflicts as open questions.
