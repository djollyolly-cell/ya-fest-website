...
- Entry condition: a worker is preparing to change Deploy Operations.
- Exit condition: `STATE.md` reflects verified code, files, docs, or external-state checks.
- Evidence/checks: read `HTML landing for claude/yandex_7f7fdde68f6a032d.html`, `website/public/robots.txt`, `HTML landing for claude/robots.txt`; compare with code/tests where relevant; record conflicts as open questions.
