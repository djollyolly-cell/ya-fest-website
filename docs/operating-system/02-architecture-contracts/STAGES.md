...
- Entry condition: a worker is preparing to change Architecture Contracts.
- Exit condition: `STATE.md` reflects verified code, files, docs, or external-state checks.
- Evidence/checks: read `website/src/components/SEO.tsx`, `website/public/robots.txt`, `website/public/sitemap.xml`, `HTML landing for claude/sitemap.xml`; compare with code/tests where relevant; record conflicts as open questions.
